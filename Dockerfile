...
RUN ARCH=$(uname -m) && \
    ...
    && tar -xzf nvim-linux-${NVIM_ARCH}.tar.gz -C /usr/local --strip-components=1 \
...
\n\
com() {\n\
  command g++ -I./ -Ilib/ -I/home/$USERNAME/kyopro-library/include -DSEN469 -std=c++20 -Wall -Wextra -Wshadow \"\$@\" \\\n\
    && ./a.out \\\n\
    && rm -f a.out\n\
}\n\
alias cjj='com j.cpp'\n\
